...
// Callback after Google API client library is loaded
function gapiLoaded() {
  gapi.client.init({
    apiKey: API_KEY,
    ...
    gisLoaded(); // Proceed to set up Google Identity Services
  }).catch(error => {
...
function gisLoaded() {
  ...
  google.accounts.id.prompt(); // Prompt user for login if not already authenticated
}

// Handles the response from authentication
function handleCredentialResponse(response) {
  if (response.credential) {
    console.log("User authenticated");
    fetchAndProcessSongsFromGoogleDrive(); // Proceed to fetch songs
  } else {
        ...
        fetch(url).then(response => response.arrayBuffer()).then(buffer => {
          jsmediatags.read(new Blob([buffer]), {
            onSuccess: tag => {
              ...
              renderSongs(userData.songs);  // Update the UI with new songs
            },
            onError: error => {
